 ...
 */
class AdminMemberGroup extends Text
{
    ...
    private function renderAsDisplay()
    {
        $this->hookRendering(function (AbstractField $field, $item, $index) {
            ...
            if (Str::contains($column, '.')) {
                $pcs = explode('.', $column);
                $pcs[0] = StrUtil::uncamelize($pcs[0]);
                $column = implode('.', $pcs);
            }
            $value = ArrayUtil::getByDotKey(ModelUtil::toArray($item), $column);
            $group = MemberGroupUtil::get($value);
            return AutoRenderedFieldValue::make($group ? $group['title'] : '<span class="ub-text-muted">-</span>');
        });
    }
}
